...
function axis(orient, scale) {
  ...
  function axis2(context) {
    ...
    line2 = line2.merge(tickEnter.append("line").attr("stroke", "currentColor").attr(x3 + "2", k4 * tickSizeInner));
    ...
    line2.attr(x3 + "2", k4 * tickSizeInner);
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i3 = 0, n2 = polygon.length; i3 < n2; ++i3) {
    ...
    for (var j3 = 0; j3 < m3; ++j3, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      var point1 = ring[j3], lambda1 = longitude(point1), phi1 = point1[1] / 2 + quarterPi, sinPhi1 = sin(phi1), cosPhi1 = cos(phi1), delta = lambda1 - lambda0, sign3 = delta >= 0 ? 1 : -1, absDelta = sign3 * delta, antimeridian = absDelta > pi2, k4 = sinPhi0 * sinPhi1;
      sum2.add(atan2(k4 * sign3 * sin(absDelta), cosPhi0 * cosPhi1 + k4 * cos(absDelta)));
...
function clipRectangle(x05, y05, x12, y12) {
  ...
  return function(stream) {
    ...
    function polygonInside() {
      var winding = 0;
      for (var i3 = 0, n2 = polygon.length; i3 < n2; ++i3) {
        for (var ring2 = polygon[i3], j3 = 1, m3 = ring2.length, point7 = ring2[0], a0, a1, b0 = point7[0], b1 = point7[1]; j3 < m3; ++j3) {
          a0 = b0, a1 = b1, point7 = ring2[j3], b0 = point7[0], b1 = point7[1];
          if (a1 <= y12) {
...
function fitExtent(projection3, extent3, object) {
  return fit(projection3, function(b3) {
    var w3 = extent3[1][0] - extent3[0][0], h3 = extent3[1][1] - extent3[0][1], k4 = Math.min(w3 / (b3[1][0] - b3[0][0]), h3 / (b3[1][1] - b3[0][1])), x3 = +extent3[0][0] + (w3 - k4 * (b3[1][0] + b3[0][0])) / 2, y3 = +extent3[0][1] + (h3 - k4 * (b3[1][1] + b3[0][1])) / 2;
...
function line_default2(x3, y3) {
  ...
  line2.defined = function(_2) {
    return arguments.length ? (defined2 = typeof _2 === "function" ? _2 : constant_default4(!!_2), line2) : defined2;
  };
...
Bundle.prototype = {
  ...
  lineEnd: function() {
    var x3 = this._x, y3 = this._y, j3 = x3.length - 1;
    if (j3 > 0) {
      var x05 = x3[0], y05 = y3[0], dx = x3[j3] - x05, dy = y3[j3] - y05, i3 = -1, t4;
      while (++i3 <= j3) {
        t4 = i3 / j3;
        this._basis.point(
          this._beta * x3[i3] + (1 - this._beta) * (x05 + t4 * dx),
          this._beta * y3[i3] + (1 - this._beta) * (y05 + t4 * dy)
        );
      }
    }
...
function point4(that, x3, y3) {
  ...
  if (that._l23_a > epsilon4) {
    ...
    x22 = (x22 * b3 + that._x1 * that._l23_2a - x3 * that._l12_2a) / m3;
    y22 = (y22 * b3 + that._y1 * that._l23_2a - y3 * that._l12_2a) / m3;
  }
...
function controlPoints(x3) {
  var i3, n2 = x3.length - 1, m3, a4 = new Array(n2), b3 = new Array(n2), r3 = new Array(n2);
  a4[0] = 0, b3[0] = 2, r3[0] = x3[0] + 2 * x3[1];
  for (i3 = 1; i3 < n2 - 1; ++i3)
    a4[i3] = 1, b3[i3] = 4, r3[i3] = 4 * x3[i3] + 2 * x3[i3 + 1];
  a4[n2 - 1] = 2, b3[n2 - 1] = 7, r3[n2 - 1] = 8 * x3[n2 - 1] + x3[n2];
  for (i3 = 1; i3 < n2; ++i3)
    m3 = a4[i3] / b3[i3 - 1], b3[i3] -= m3, r3[i3] -= m3 * r3[i3 - 1];
  a4[n2 - 1] = r3[n2 - 1] / b3[n2 - 1];
  for (i3 = n2 - 2; i3 >= 0; --i3)
    a4[i3] = (r3[i3] - a4[i3 + 1]) / b3[i3];
  b3[n2 - 1] = (x3[n2] + a4[n2 - 1]) / 2;
  for (i3 = 0; i3 < n2 - 1; ++i3)
    b3[i3] = 2 * x3[i3 + 1] - a4[i3 + 1];
  return [a4, b3];
}
...
function maybeApplyInterval(V2, scale) {
  const t4 = maybeIntervalTransform(scale?.interval, scale?.type);
  return t4 ? map2(V2, t4) : V2;
}
...
function inherit2(options = {}, ...rest) {
  let o3 = options;
  for (const defaults10 of rest) {
    for (const key in defaults10) {
      if (o3[key] === void 0) {
        const value = defaults10[key];
        if (o3 === options)
          o3 = { ...o3, [key]: value };
        else
          o3[key] = value;
...
function filterTransform(value) {
  return (data, facets) => {
    ...
    return { data, facets: facets.map((I2) => I2.filter((i3) => V2[i3])) };
  };
}
...
function sortValue(value) {
  ...
  return (data, facets, channels) => {
    ...
    const compareValue = (i3, j3) => order(V2[i3], V2[j3]);
...
function reduceProportion(value, scope) {
  return value == null ? { scope, label: "Frequency", reduceIndex: (I2, V2, basis2 = 1) => I2.length / basis2 } : { scope, reduceIndex: (I2, V2, basis2 = 1) => sum(I2, (i3) => V2[i3]) / basis2 };
}
...
function groupAesthetics({
  ...
  return [AL, tip2 ? void 0 : T2, F2, FO, S2, SO, SW, O2, H2].filter((c6) => c6 !== void 0);
}
...
function* groupIndex(I2, position2, mark, channels) {
  ...
  for (const G of Z ? groupZ(I2, Z, z3) : [I2]) {
    let Ag;
    let Gg;
    out:
      for (const i3 of G) {
        ...
        if (Ag === void 0) {
          if (Gg)
            yield Gg;
          Ag = A5.map((c6) => keyof2(c6[i3])), Gg = [i3];
          continue;
        }
        Gg.push(i3);
        for (let j3 = 0; j3 < A5.length; ++j3) {
          const k4 = keyof2(A5[j3][i3]);
          if (k4 !== Ag[j3]) {
            yield Gg;
            Ag = A5.map((c6) => keyof2(c6[i3])), Gg = [i3];
            continue out;
...
function facetIndex(V2) {
  ...
  if (!I2)
    indexCache.set(V2, I2 = new InternMap(map2(V2, (v3, i3) => [v3, i3])));
  return I2;
}
...
var Mark = class {
  constructor(data, channels = {}, options = {}, defaults10) {
    const {
      facet = "auto",
      facetAnchor,
      fx,
    ...
    if (defaults10 !== void 0)
      channels = { ...styles(this, options, defaults10), ...channels };
...
function applyGroupedMarkers(path2, mark, { stroke: S2 }, context) {
  return applyMarkersColor(path2, mark, S2 && (([i3]) => S2[i3]), context);
}
...
function exposeLegends(scales, context, defaults10 = {}) {
  return (key, options) => {
    if (!legendRegistry.has(key))
      throw new Error(`unknown legend type: ${key}`);
    if (!(key in scales))
      return;
    return legendRegistry.get(key)(scales[key], legendOptions(context, defaults10[key], options), (key2) => scales[key2]);
  };
}
...
function maybeAxis(axes, axis2, axisType, primary, secondary, defaults10, options) {
  if (!axis2)
    return;
  const both = isBoth(axis2);
  options = axisOptions(both ? primary : axis2, defaults10, options);
...
function axisOptions(anchor, defaults10, {
  line: line2 = defaults10.line,
  ticks: ticks2,
  tickSize,
  tickSpacing,
  tickPadding,
  ...
  ariaLabel,
  ariaDescription,
  label = defaults10.label,
  labelAnchor,
  labelArrow = defaults10.labelArrow,
  labelOffset
}) {
  return {
    anchor,
      ...
      for (const facet of facets) {
        ...
        for (const [f3, I2] of maybeGroup(facet, G)) {
          for (const [k5, g3] of maybeGroup(I2, K2)) {
            for (const [b3, extent3] of bin(g3)) {
...
function maybeBin(options) {
  ...
  const bin = (data) => {
    ...
    if (isTemporal(V2) || isTimeThresholds(thresholds)) {
      ...
      let t4 = typeof thresholds === "function" && !isInterval(thresholds) ? thresholds(V2, min4, max3) : thresholds;
      if (typeof t4 === "number")
    ...
    E2.bin = (cumulative < 0 ? bin1cn : cumulative > 0 ? bin1cp : bin1)(E2, T2, V2);
    return E2;
  };
...
function bin1(E2, T2, V2) {
  T2 = coerceNumbers(T2);
  return (I2) => {
    const B4 = E2.map(() => []);
    for (const i3 of I2)
      B4[bisect_default(T2, V2[i3]) - 1]?.push(i3);
    return B4;
  };
}
function bin1cp(E2, T2, V2) {
  const bin = bin1(E2, T2, V2);
  return (I2) => {
    const B4 = bin(I2);
...
function exclusiveFacets(data, facets) {
  ...
  if (overlaps === 0)
    return { data, facets };
  data = slice2(data);
  const R = data[reindex] = new Uint32Array(n2 + overlaps);
  facets = facets.map((facet) => slice2(facet, Uint32Array));
  let j3 = n2;
  O2.fill(0);
  for (const facet of facets) {
    for (let k4 = 0, m3 = facet.length; k4 < m3; ++k4) {
      const i3 = facet[k4];
      if (O2[i3])
        facet[k4] = j3, data[j3] = data[i3], R[j3] = i3, ++j3;
      else
        R[i3] = i3;
      O2[i3] = 1;
    }
...
function stack(x3, y3 = one2, kx2, ky2, { offset: offset2, order, reverse: reverse2 }, options) {
  ...
  return [
    basic(options, (data, facets, plotOptions) => {
      ...
      const facetstacks = [];
      for (const facet of facets) {
        const stacks = X4 ? Array.from(group(facet, (i3) => X4[i3]).values()) : [facet];
        if (compare)
        ...
        for (const stack2 of stacks) {
          ...
          for (const i3 of stack2) {
            const y4 = Y3[i3];
            if (y4 < 0)
              yn = Y23[i3] = (Y13[i3] = yn) + y4;
            else if (y4 > 0)
              yp = Y23[i3] = (Y13[i3] = yp) + y4;
            else
              Y23[i3] = Y13[i3] = yp;
          }
        }
...
function offsetWiggle(facetstacks, Y12, Y22, Z) {
  for (const stacks of facetstacks) {
    ...
    for (const stack2 of stacks) {
      let j3 = -1;
      const Fi = stack2.map((i3) => Math.abs(Y22[i3] - Y12[i3]));
      const Df = stack2.map((i3) => {
        j3 = Z ? Z[i3] : ++j3;
        const value = Y22[i3] - Y12[i3];
        const diff = prev.has(j3) ? value - prev.get(j3) : 0;
        prev.set(j3, value);
        return diff;
      });
      ...
      const s1 = sum(Fi);
      if (s1)
        y3 -= sum(Fi, (d3, i3) => (Df[i3] / 2 + Cf1[i3]) * d3) / s1;
    }
    offsetZero(stacks, Y12, Y22);
  }
  offsetCenterFacets(facetstacks, Y12, Y22);
...
function offsetCenterFacets(facetstacks, Y12, Y22) {
  ...
  const m3 = facets.map((I2) => (min(I2, (i3) => Y12[i3]) + max(I2, (i3) => Y22[i3])) / 2);
  const m0 = min(m3);
  for (let j3 = 0; j3 < n2; j3++) {
    const p3 = m0 - m3[j3];
    for (const i3 of facets[j3]) {
      Y12[i3] += p3;
      Y22[i3] += p3;
    }
  }
...
function orderY(compare) {
  return (data, X3, Y3) => (i3, j3) => compare(Y3[i3], Y3[j3]);
}
function orderZ(compare) {
  return (data, X3, Y3, Z) => (i3, j3) => compare(Z[i3], Z[j3]);
}
...
function orderAccessor(f3) {
  return (data) => {
    const O2 = valueof(data, f3);
    return (i3, j3) => ascendingDefined2(O2[i3], O2[j3]);
  };
}
function orderComparator(f3) {
  return (data) => (i3, j3) => f3(data[i3], data[j3]);
}
...
function orderZDomain(compare, domain) {
  return (data, X3, Y3, Z) => {
    ...
    const map3 = new InternMap(domain(data, X3, Y3, Z).map((d3, i3) => [d3, i3]));
    return (i3, j3) => compare(map3.get(Z[i3]), map3.get(Z[j3]));
  };
}
...
var AbstractBar = class extends Mark {
  constructor(data, channels, options = {}, defaults10) {
    super(data, channels, options, defaults10);
...
var BarY = class extends AbstractBar {
  ...
  _y({ y: y3 }, { y1: Y12, y2: Y22 }, { marginTop }) {
    const { insetTop } = this;
    return isCollapsed(y3) ? marginTop + insetTop : (i3) => Math.min(Y12[i3], Y22[i3]) + insetTop;
  }
  _height({ y: y3 }, { y1: Y12, y2: Y22 }, { marginTop, marginBottom, height }) {
    const { insetTop, insetBottom } = this;
    return isCollapsed(y3) ? height - marginTop - marginBottom - insetTop - insetBottom : (i3) => Math.max(0, Math.abs(Y22[i3] - Y12[i3]) - insetTop - insetBottom);
  }
};
...
var Line = class extends Mark {
  constructor(data, options = {}) {
    const { x: x3, y: y3, z: z3, curve, tension } = options;
    super(
      data,
      {
        x: { value: x3, scale: "x" },
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g3) => g3.selectAll().data(groupIndex(index2, [X3, Y3], this, channels)).enter().append("path").call(applyDirectStyles, this).call(applyGroupedChannelStyles, this, channels).call(applyGroupedMarkers, this, channels, context).attr(
        "d",
        curve === curveAuto && context.projection ? sphereLine(context.projection, X3, Y3) : line_default2().curve(curve).defined((i3) => i3 >= 0).x((i3) => X3[i3]).y((i3) => Y3[i3])
      )
    ).node();
  }
};
...
function ColumnSelect(props) {
  ...
  return /* @__PURE__ */ y("div", null, /* @__PURE__ */ y("div", { style: "display: grid; grid-template-columns: 60px auto;" }, /* @__PURE__ */ y("div", null, /* @__PURE__ */ y("span", null, title)), /* @__PURE__ */ y("div", null, /* @__PURE__ */ y(
    "select",
    {
      value,
      onChange: (e3) => setValue(e3.target.value)
    },
    columns.map((c6) => /* @__PURE__ */ y("option", { key: c6 }, c6))
  ))));
}
function DotEditor(props) {
  const [x3, setX] = h2(props.options.x);
  const [y3, setY] = h2(props.options.y);
  const [fill, setFill] = h2(props.options.fill);
  p2(() => {
  ...
  ), /* @__PURE__ */ y("input", { name, id: `${name}-column`, type: "radio" }), /* @__PURE__ */ y("label", { for: `${name}-column` }, "Column"), /* @__PURE__ */ y("input", { name, id: `${name}-value`, type: "radio" }), /* @__PURE__ */ y("label", { for: `${name}-column` }, "Value"));
}
...
function BarEditor(props) {
  ...
  const [y3, setY] = h2(props.options.y);
  p2(() => {
    props.onUpdate({ x: x3, y: y3, tip: true });
  }, [x3, y3]);
  return /* @__PURE__ */ y("div", null, /* @__PURE__ */ y(
    ColumnSelect,
    {
...
function LinearRegressionYEditor(props) {
  const [x3, setX] = h2(props.options.x);
  const [y3, setY] = h2(props.options.y);
  const [stroke, setStroke] = h2(props.options.stroke);
  p2(() => {
    props.onUpdate({ x: x3, y: y3, stroke, tip: true });
  }, [x3, y3, stroke]);
  return /* @__PURE__ */ y("div", null, /* @__PURE__ */ y(
    ColumnSelect,
    {
...
function MarkEditor(props) {
  let [mark, setMark] = h2(props.initalMark);
  let [options, setOptions] = h2(props.initionalOptions);
  p2(() => {
  ...
  function render() {
    switch (mark) {
      ...
      case "linear-regression-y" /* LinearRegressionY */:
        return /* @__PURE__ */ y(
  ...
  return /* @__PURE__ */ y("div", { style: "border: 1px solid black;" }, /* @__PURE__ */ y("div", { style: "" }, "Mark type:", " ", /* @__PURE__ */ y(
    "select",
    {
      value: mark,
      onChange: (e3) => setMark(e3.target.value)
    },
    /* @__PURE__ */ y("option", { value: "dot" /* Dot */ }, "Dot"),
    /* @__PURE__ */ y("option", { value: "linear-regression-y" /* LinearRegressionY */ }, "Linear Regression (y)"),
    /* @__PURE__ */ y("option", { value: "bar-y" /* BarY */ }, "Bar (y)"),
    /* @__PURE__ */ y("option", { value: "area-y" /* AreaY */ }, "Area (y)"),
    /* @__PURE__ */ y("option", { value: "line-y" /* LineY */ }, "Line (y)")
  )), /* @__PURE__ */ y("div", { style: "padding: .5rem 1rem;" }, render()));
}
function interestingColumns(columns, sample) {
  let x3, y3;
  for (const column2 of columns) {
    if (column2 === "rowid" || column2 === "id")
      continue;
    if (typeof sample[column2] === "number") {
      if (x3 === void 0) {
        x3 = column2;
      } else if (y3 === void 0) {
        y3 = column2;
...
function PlotEditor(props) {
  const [show, setShow] = h2(props.initalMarks !== null);
  ...
  return /* @__PURE__ */ y("div", null, /* @__PURE__ */ y("strong", null, "Marks"), marks2.map((mark, idx) => /* @__PURE__ */ y(
    MarkEditor,
    {
      columns: props.columns,
      initalMark: mark.mark,
      initionalOptions: mark.options,
      onUpdate: (mark2, options) => {
        setMarks(marks2.map((d3, i3) => i3 === idx ? { mark: mark2, options } : d3));
      }
    }
  )), /* @__PURE__ */ y(
    "button",
    {
      onClick: () => {
        ...
        let x3, y3;
        if (marks2.length) {
          x3 = marks2[marks2.length - 1].options.x;
          y3 = marks2[marks2.length - 1].options.y;
        } else {
          [x3, y3] = interestingColumns(props.columns, props.data[0]);
        }
        setMarks([
          ...marks2,
          {
...
function Preview(props) {
  ...
  p2(() => {
    ...
    const p3 = plot({
      color: { legend: true },
      marks: props.marks.map((m3) => {
        switch (m3.mark) {
          case "dot" /* Dot */:
            return dot(props.data, m3.options);
          case "linear-regression-y" /* LinearRegressionY */:
            return linearRegressionY(props.data, m3.options);
          case "line-y" /* LineY */:
            return lineY(props.data, m3.options);
          case "bar-y" /* BarY */:
            return barY(props.data, m3.options);
        }
      })
    });
  ...
  return /* @__PURE__ */ y("div", null, /* @__PURE__ */ y("div", { ref: target }), /* @__PURE__ */ y("a", { href: url }, "Link to this plot"));
}
async function main() {
  const dataUrl = new URL(
    window.location.origin + window.location.pathname + ".json" + window.location.search
  );
  if (!dataUrl.searchParams.has("_size")) {
    dataUrl.searchParams.set("_size", "max");
  }
  const data = await fetch(dataUrl).then(
    (r3) => r3.json()
  );
  const target = document.querySelector("form.sql") || document.querySelector("form.filters");
  const root2 = target.insertAdjacentElement(
    "afterend",
    document.createElement("div")
  );
  const columns = Object.keys(data.rows[0]);
  const url = new URL(window.location.href);
  const initalMarks = url.searchParams.has("_plot-mark") ? url.searchParams.getAll("_plot-mark").map((d3) => JSON.parse(d3)) : null;
  B(
    /* @__PURE__ */ y("div", { className: "datasette-plot" }, /* @__PURE__ */ y(
      PlotEditor,
      {
        data: data.rows,
        columns,
    ...
    ), data.next !== null ? /* @__PURE__ */ y("div", null, "Warning: not all table rows returned, only ", data.rows.length, " rows") : null),
    root2
  );
}
